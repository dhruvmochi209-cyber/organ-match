...
@recipient_bp.route('/profile', methods=['GET'])
@jwt_required()
def get_profile():
...
def register_recipient():
    ...
    required = ['full_name', 'blood_type', 'age', 'organ_needed', 'urgency', 'location']
    if not all(k in data for k in required):
        return jsonify({'error': 'Missing required fields'}), 400

    if Recipient.query.filter_by(user_id=user.id).first():
        return jsonify({'error': 'Recipient profile already exists'}), 409
    ...
    # 🧹 ROBUST FAIL-SAFE: Clear any orphan reports to ensure the recipient starts with a clean clinical history.
    # Keeps only the single report uploaded during this registration.
    ...
    db.session.commit()
    log_audit(db, user.id, 'RECIPIENT_REGISTERED', f"Recipient profile created for {data['full_name']}")
    return jsonify({'message': 'Recipient profile created', 'recipient': recipient.to_dict()}), 201

@recipient_bp.route('/upload-report', methods=['POST'])
@jwt_required()
def upload_report():
    user = get_current_user()
    ...
    organ = request.form.get('organ') # organ_needed for recipient
    ...
    db.session.commit()
    log_audit(db, user.id, 'REPORT_UPLOADED', f"Medical report ({organ}) uploaded by recipient {user.id}")
    return jsonify({'message': 'Report verified and uploaded successfully', 'report': report.to_dict()}), 201

@recipient_bp.route('/reports', methods=['GET'])
@jwt_required()
def get_reports():
    user = get_current_user()
...
@recipient_bp.route('/all', methods=['GET'])
@jwt_required()
